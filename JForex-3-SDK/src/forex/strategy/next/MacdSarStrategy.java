...
public class MacdSarStrategy implements IStrategy {
	...
	public void onBar(Instrument instrument, Period period, IBar askBar, IBar bidBar) throws JFException {
		...
		double[] adx = indicators.adx(instrument, Period.FIFTEEN_MINS, offerSide, 14, indicatorFilter, 3,
				askBar.getTime(), 0);
		 ...
		 **********************************************************************************************/
		double[] sar = indicators.sar(instrument, period, offerSide, 0.02, 0.2, indicatorFilter, 3, askBar.getTime(),
				0);
		boolean isSARBuy = (sar[0] < prevPrevBar.getLow() && sar[1] < prevBar.getLow() && sar[2] < askBar.getLow());
		boolean isSARSell = (sar[0] > prevPrevBar.getHigh() && sar[1] > prevBar.getHigh() && sar[2] > askBar.getHigh());

		double[][] macd = indicators.macd(instrument, this.selectedPeriod, offerSide, appliedPrice, 12, 26, 9,
				indicatorFilter, 2, askBar.getTime(), 0);
		final int HIST = 2;
		boolean isSMABuy = false, isSMASell = false;

		// [2.194630726983803E-5, -2.5972827843041108E-5]
		if (macd[HIST][1] > 0 && macd[HIST][0] <= 0) {
			isSMABuy = true;
		}
	...
	private IOrder submitOrder(final OrderCommand orderCmd, final IBar bidBar, final IBar askBar) throws JFException {
		...
		if (orderCmd == OrderCommand.BUY) {
			...
			final double atr = indicators.atr(instrument, Period.ONE_HOUR, OfferSide.BID, 21, 1);
			...
			// stopLossPrice = calculateSellStopLossFromFib(askBar);
			// takeProfitPrice = calculateTakeProfitFromFibSell(askBar);
			//
			// double oldTakeProfit = askBar.getClose() - getPipPrice(this.takeProfitPips);
			// if (oldTakeProfit != takeProfitPrice) {
			// takeProfitPrice = oldTakeProfit;
			// }
			//
			// double oldStopLoss = askBar.getClose() + getPipPrice(this.stopLossPips);
	 ...
	 * @throws JFException
	 */
	private double calculateBuyTakeProfitFromFib(final IBar askBar) throws JFException {
		double[][] fibPivot = indicators.fibPivot2(instrument, Period.FOUR_HOURS, offerSide, indicatorFilter, 1,
				askBar.getTime(), 0);

		// P - R1 - R2 - R3
		double[] resistances = new double[] { fibPivot[0][0], fibPivot[1][0], fibPivot[3][0], fibPivot[5][0] };
		double currentValue = askBar.getClose();
		...
		// double[][] donchian = indicators.donchian(instrument, selectedPeriod,
		// offerSide, dcTimePeriod, indicatorFilter,
		// 4, askBar.getTime(), 0);
	}

	private double calculateBuyStopLossFromFib(final IBar askBar) throws JFException {
		...
		// return history.g*etLastTick(this.instrument).getAsk() -
		// getPipPrice(this.stopLossPips);
	}

	private double calculateTakeProfitFromFibSell(final IBar askBar) throws JFException {
		double[][] fibPivot = indicators.fibPivot2(instrument, Period.FOUR_HOURS, offerSide, indicatorFilter, 1,
				askBar.getTime(), 0);
		// Just on Support 2
	...
	private double calculateSellStopLossFromFib(IBar askBar) throws JFException {
		double[][] fibPivot = indicators.fibPivot2(instrument, Period.FOUR_HOURS, offerSide, indicatorFilter, 1,
				askBar.getTime(), 0);
