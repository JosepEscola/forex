...
public class StochSARRestrictiuStrategy implements IStrategy {
	...
	public void onBar(Instrument instrument, Period period, IBar askBar, IBar bidBar) throws JFException {
		...
		int K = 0;
		int D = 1;
		double[][] stoch = indicators.stoch(instrument, selectedPeriod, offerSide, fastKPeriod, slowKPeriod,
				slowKMaType, slowDPeriod, slowDMaType, indicatorFilter, 1, askBar.getTime(), 0);

		boolean isOversold = false, isOverBought = false;
		if (stoch[K][0] < 20 && stoch[D][0] < 20) {
			isOversold = true;
		} else if (stoch[K][0] > 80 && stoch[D][0] > 80) {
			isOverBought = true;
		}
		...
		boolean isADX = (adx[2] >= 30) || (adx[0] < adx[1] && adx[1] < adx[2] && adx[2] >= 20);
		...
		if (isSMABuy && isADX && !isOverBought && isSARBuy && isRiskManagementOK()) {

			long timeBetweenOrders = (lastOrder == null ? 0 : (askBar.getTime() - lastOrder.getCreationTime()));
			long timeToWait = period.getInterval() * 4; // 1 Hour
		...
		if (isSMASell && isADX && !isOversold && isSARSell && isRiskManagementOK()) {
	...
	private IOrder submitOrder(final OrderCommand orderCmd, final IBar bidBar, final IBar askBar) throws JFException {
		...
		final Period atrPeriod = Period.ONE_HOUR;// Period.FOUR_HOURS;
